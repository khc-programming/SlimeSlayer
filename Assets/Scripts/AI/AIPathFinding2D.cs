...
public class AIPathFinding2D : BaseAIPath
{
    ...
    public void Find(AIPath2D sk, AIPath2D t, List<AINode2D> s = null)
    {
        ...
        if (execute == false)
        {
            ...
            //Finding(this.seeker, this.target);
    ...
    //public void Finding(AIPath2D seeker, AIPath2D target , AIPath2D subWall = null)
    //{

    //    print("길찾기 시작 = " + seeker.name);
    //    OnReset();
    ...
    //            AIorderList.RemoveAt(0);
    //            Finding(orderSeeker, orderTarget , subWall);
    //        }
    //        else
    //        {
    ...
    //        {
    //            // 벽과 닫힘목록 리스트에 들어가 있는 노드는 무시하도록 처리합니다.
    ...
    //                // 이웃노드의 부모를 현재의 기준노드로 설정합니다.
    //                neighbours[i].parents = currNode2D;

    //                // 오픈노드 목록에 등록되지 않은 상태라면 오픈노드리스트에 등록합니다.
    //                if (openedNode2D.Contains(neighbours[i]) == false)
    ...
    //            // 현재 기준노드를 오픈노드리스에 있는 최소 비용값으로 설정합니다.
    ...
    //        AIPath2D orderTarget = AIorderList[0].target;
    //        AIorderList.RemoveAt(0);
    //        Finding(orderSeeker, orderTarget);
    //    }
    //    else
    //    {
    //        execute = false;
